...
module sparsecenter( h, e )
{
    ...
    union()
        {
    cylinder( r = radius608+e, h=h,center=true, $fn=200);
    rotate([0,0,45])
    cube( [85,2*e,h],center=true);
    rotate([0,0,-45])
    cube( [85,2*e,h],center=true);  
        }
    union()
        {
    cylinder( r = radius608, h=h+0.01,center=true, $fn=200);
             attachpulleyholes(h+1);
        }
    }
...
teeth = 140;			// Number of teeth, standard Mendel T5 belt = 8, gives Outside Diameter of 11.88mm
...
cylinder( r =38, h = 100, center=true,$fn=200);
